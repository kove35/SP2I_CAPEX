...
- **Fixture `admin_auth` corrigée** (`tests/conftest.py`) : fixture limitée aux tests qui surcharge `get_current_user` via `app.dependency_overrides` (ADMIN de test) et **restaure les overrides** dans un `finally`. L'authentification applicative n'est pas modifiée. Résultat : **30 passed** ; `test_excel_sync_datetime_serialization` **passe la collecte** et atteint le code applicatif. Il échoue encore au **runtime** uniquement parce que la base isolée n'a pas la chaîne de migrations complète (`psycopg.errors.UndefinedTable: relation "dim_lot" does not exist` → 422) : ce point relève de la **validation de la chaîne de migrations**, documentée séparément de la recette (voir ci-dessous).
