...
- **E2E Playwright** : la suite `workflow-project.spec.js` (27 tests, mock API selon le pattern du dépôt) passe **27/27** contre le vrai serveur frontend Vite auto-démarré → le flux complet (landing → projet → cockpit → DQE → simulation → approvisionnement → chantier) reste fonctionnel après les correctifs. Le parcours navigateur « vrai backend local » est couvert séparément par `real-api-recipe.spec.js` (5/5, config dédiée, aucun mock d'API métier).
- **Fixture `admin_auth` corrigée** (`tests/conftest.py`) : fixture limitée aux tests qui surcharge `get_current_user` via `app.dependency_overrides` (ADMIN de test) et **restaure les overrides** dans un `finally`. L'authentification applicative n'est pas modifiée. Le test `test_excel_sync_datetime_serialization` effectue une **vraie synchronisation** sur la base isolée dédiée `sp2i_capex_sync_test` (schéma dédié couvrant les routes exercées, reconstruit depuis le DDL de `cloud_migrations` via `tests/recette/prepare_sync_db.py`). Résultat : **31 passed** (suite backend entièrement verte sur base isolée, test de sync inclus).
...
- **Tests navigateur avec API réelle** : ✅ **5 scénarios validés**. Parcours `08_FRONTEND/tests/e2e/real-api-recipe.spec.js` (config dédiée `playwright.realapi.config.js`, backend + frontend réels, aucun `page.route()` sur l'API métier) : connexion, projet A (CAPEX Direct = 3 000 FCFA), filtre niveau RDC (1 000 FCFA), projet B (500 FCFA), projet vide C (aucun montant résiduel) et ratio indisponible sur D (niveau S1 → « Indisponible pour ce perimetre »). Blocage initial levé : `/analytics/filters` renvoyait HTTP 500 car (1) la vue `vw_fact_metre_current` manquait dans le schéma de recette et (2) `_piece_filter_options` n'injectait pas le bind `:projet` (bug corrigé + test de régression). Commande dédiée : `$env:VITE_SP2I_USE_V6_FINANCIALS='true'; npx playwright test --config=playwright.realapi.config.js`.
...
- **Défauts visuels recette manuelle (A/B/C/D)** : (1) encadré « Projet immobilier » affichait toujours « Complexe immobilier Mpemba » — cause : repli systématique de `getProjectContext` sur `PROJECT_CONTEXT` pour tout code inconnu et appels avec le seul identifiant technique. Correction : le contexte projet provient des informations applicatives (`activeProjectDetails`, store unique) et le repli Mpemba n'est appliqué qu'au projet par défaut `PROJET_MPEMBA` ; champs facultatifs (localisation/type) masqués quand absents. (2) Projet vide (C) : la grille basculait en cartes V5 (« Budget initial »…) car le mode V6 dépendait de la présence d'un montant non nul ; désormais le cockpit force le mode V6 par configuration (`V6_FINANCIALS_ENABLED`) et ne fusionne plus les KPI V5 en V6. (3) État vide honnête : bannière « Aucune donnée pour ce projet » + cartes V6 conservées (additifs à zéro, ratios indisponibles) ; réponse vide (contrat `SUCCESS`) distinguée d'une erreur API ; état « aucun résultat pour les filtres » distinct (réinitialisation disponible via le bouton existant du panneau de filtres, projet conservé). (4) D/S1 et projets vides : suppression des mini-courbes et variations fabriquées (KpiSparkline/KpiTrendBadge n'affichent rien sans série/delta réels, `points: []` et `delta: null` en grille), un vrai zéro reste affiché comme zéro.
- **Validations défauts visuels** : build Vite ✅ ; spec navigateur API réelle (`real-api-recipe.spec.js`) ✅ 1 passed — connexion, A (nom « Projet A synthetique » affiché, aucune occurrence « Mpemba », CAPEX Direct 3 000 FCFA), **rechargement : nom et contexte A conservés, toujours sans « Mpemba »**, filtre niveau RDC (1 000 FCFA), B (500 FCFA), C vide (message « Aucune donnée pour ce projet » + mention « Non évalué » + carte V6 CAPEX Direct 0 FCFA), D/S1 (« Indisponible pour ce perimetre » sur « Par appartement » **sans `svg.kpi-sparkline`**).
- **Vérifications complémentaires (lot visuel, clôture)** : suite Playwright standard `workflow-project.spec.js` **rejouée après les changements UI → 27/27 (1,2 min)** ; build Vite ✅ ; spec navigateur API réelle ✅ 1 passed (A/rechargement/RDC/B/C vide/D-S1). Panneaux annexes du projet vide corrigés (plus d'évaluation sans données) : `CapexTimeline`, `RiskMatrix` et `InsightsPanel` affichent désormais « Aucune donnee pour ce projet » au lieu de tendances/risque/ROI calculés sur des valeurs par défaut.
- **Périmètre filtré du dashboard V6 — corrigé (lot dédié)** : la vue tableau `vw_dashboard_direction_v6_scoped` ignorait les filtres spatiaux (le dashboard renvoyait l'agrégat projet sous filtres). Correction dans le repository : `get_dashboard_direction_v6` agrège désormais les lignes `vw_fact_metre_financial_v6` **après application des filtres** (mêmes taux que le summary scopé, aucun changement de formule financière). Une combinaison valide sans résultat renvoie `SUCCESS`, `nb_lignes=0`, `nb_lots=0`, tableau vide, montants additifs 0, ratios `null`, confiance `null` ; la confiance n'est plus forgée à HIGH/95 sans lignes. **Contrat API inchangé en structure** (mêmes clés) ; `nb_lignes` et `nb_lots` reflètent désormais le périmètre filtré (documenté).
- **Validations périmètre filtré (lot précédent)** : régression PostgreSQL `tests/integration/test_v6_dashboard_filtered_scope.py` → **22/22 OK** ; suite backend pytest ✅ **31 passed** ; scénario navigateur réel (stack :8001/:5174) ✅ — A→RDC 1 000 → RDC+LOT_CVC vide (SUCCESS/0/vide/ratios null) → retrait LOT_CVC → RDC 1 000 ; A=3000/B=500/C vide/D-S1 conservés.
- **Correctif C2 — panneaux secondaires V6 (campagne)** : le tableau détail est désormais alimenté par les **lignes financières V6** (`payload.lines` ajouté à `/analytics/v6/dashboard` via `AnalyticsResponse.lines`, grain `vw_fact_metre_financial_v6`) et non par les agrégats par lot ; somme directe visible = 3 000 FCFA, 4 lignes, désignations et décisions réelles. Sankey/heatmap utilisent `capex_local` (décision d'achat, budget direct) ; waterfall avec unités adaptatives (FCFA/k/M) et suppression des lots/fournisseurs inventés ; risque/historique : messages distincts « Aucune évaluation de risque disponible » / « Aucun historique disponible ». Aucune formule financière modifiée. Campagne : **C1–C6 6/6** (backend réel :8001) ; **C7 simulation 2/2** (erreur ≠ vide ; null ≠ zéro) ; suite standard 27/27 ; build Vite OK.
- **Réinitialiser conserve le projet actif — corrigé (régression)** : `resetFilters(keepProjet)` conserve le projet courant ; le moteur re-synchronise tout écart `filters.projet != projet actif`. A→RDC→LOT_CVC vide→**Réinitialiser** → 3 000 FCFA, message vide disparu, contexte conservé après rechargement (scénario navigateur réel ✅).
