...
- **Limite reproduite (bloquant, cause précise)** : la distinction « aucun résultat pour les filtres sélectionnés » n'est pas fiable sur le contrat V6 actuel : `/analytics/v6/dashboard?projet=PROJET_A&niveau=RDC&lot=LOT_CVC` (combinaison sans ligne) renvoie encore `nb_lignes=4`, `table_len=3`, `status=SUCCESS` (agrégat projet conservé par le dashboard V6 sous filtres). La détection par `nb_lignes` est donc inopérante pour ce cas ; l'état « projet vide » (contrat réellement sans lignes) reste correct. Un correctif fiable nécessiterait un changement de contrat du dashboard V6 (hors périmètre « ne pas modifier les calculs financiers validés »). Vérifications « erreur API » (grille masquée + message) et « mode V5 » (flag désactivé) : vérifiées par code ; non automatisées dans ce lot (pas de jeu de données V5/erreur dans la recette).
