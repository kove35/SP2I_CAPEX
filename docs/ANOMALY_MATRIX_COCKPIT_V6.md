...
- **Suite backend pytest** : `test_project_isolation.py`, `test_financial_source.py`, `test_json_safe.py`, `test_auth_security.py`, `test_upload_security.py`, `test_workflow_engine.py` → **30 passed**. `test_sync_datetime_serialization.py::test_excel_sync_datetime_serialization` est en **erreur de collecte** (`fixture 'admin_auth' not found`) : le test déclare `pytestmark = pytest.mark.usefixtures("admin_auth")` mais **aucune fixture `admin_auth` n'est définie** dans le dépôt (pas de `conftest.py`). C'est un écart de harnais de test préexistant, indépendant des correctifs V6 (le test requiert aussi une stack complète : base réelle + endpoint d'upload).
...
- Validation automatisée : build Vite ✅, E2E Playwright `workflow-project.spec.js` ✅ (27/27), suite backend pytest ✅ (30 passed), test d'intégration Anomalie G ✅ (PostgreSQL isolé, incluant le test de parité et la déduplication surface).
