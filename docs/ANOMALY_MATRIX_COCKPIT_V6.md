...
- **Validations périmètre filtré** : régression PostgreSQL `tests/integration/test_v6_dashboard_filtered_scope.py` → **22/22 OK** (A=3000/4 lignes/3 lots, RDC=1000/2 lots, RDC+LOT_CVC vide SUCCESS/0/vide/ratios null, B=500 isolé, projet E à lignes nulles nb=2 non vide) ; suite backend pytest ✅ **31 passed** ; scénario navigateur réel (stack corrigée : backend :8001 + frontend V6 :5174) ✅ **1 passed** — A → RDC 1 000 → +LOT_CVC « Aucune donnée pour les filtres sélectionnés » → retrait LOT_CVC → RDC 1 000 ; A/B/C/D inchangés.
