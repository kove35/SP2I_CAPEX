...
- **Réinitialiser conserve le projet actif — corrigé (régression)** : cause — `resetFilters` du store ramenait `filters.projet` à la valeur par défaut `PROJET_MPEMBA`, et le garde « déjà synchronisé » du moteur (`lastSyncedProject`) empêchait la re-synchronisation vers le projet actif → après Projet A→RDC→LOT_CVC→Réinitialiser, le cockpit interrogeait `PROJET_MPEMBA` (CAPEX 0, « Aucune donnée pour ce projet »). Correction : `resetFilters(keepProjet)` conserve le projet courant (reset secondaire uniquement) dans `useCrossFiltering.reset` ; le moteur re-synchronise désormais tout écart `filters.projet != projet actif` (garde supprimée). Aucun calcul financier modifié. Scénario navigateur réel ajouté (A→RDC→LOT_CVC vide→**Réinitialiser**) : retour **3 000 FCFA**, message vide disparu, « 4 lignes à analyser » visible, ratio « Par appartement = 1 329 FCFA » (jamais artificiellement 0), contexte A conservé après rechargement. Résultats : spec navigateur réel :5174 ✅ **1 passed** ; suite Playwright standard ✅ **27/27** ; build Vite ✅.
