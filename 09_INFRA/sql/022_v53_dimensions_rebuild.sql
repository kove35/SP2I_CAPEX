...
-- Migration: 022_v53_dimensions_rebuild.sql
--
-- Objectif:
...
--   et dim_article_bpu ;
-- - conserver les contrats de colonnes existants pour Power BI ;
...
-- Vue active des lots.
-- Contrat historique conserve en tete:
-- lot_id, lot, ordre_lot, created_at, updated_at.
-- Colonnes enrichies ajoutees en fin:
-- lot_libelle, description, is_active.
...
-- Vue active des sous-lots.
-- Contrat historique conserve en tete:
-- sous_lot_id, lot_id, description, created_at, updated_at.
-- Colonnes enrichies ajoutees en fin pour les usages Power BI modernes.
...
SELECT
    ...
    now()::timestamptz AS created_at,
    now()::timestamptz AS updated_at,
    sous_lot_id AS sous_lot_code,
    description AS sous_lot,
    lot_id::varchar(255) AS lot,
    DENSE_RANK() OVER (ORDER BY lot_id, sous_lot_id)::integer AS ordre_sous_lot,
    true::boolean AS is_active
FROM sous_lots;
...
-- Vue active des articles BPU.
-- Contrat historique Power BI conserve:
-- article_bpu_id, code_article, lot, sous_lot, designation, unite,
-- is_bim_compatible, created_at, updated_at, article_id, lot_id,
-- sous_lot_id, marque, is_active.
