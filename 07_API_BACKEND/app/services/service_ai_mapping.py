...
class ServiceAIMapping:
    ...
    def analyser_excel(self, contenu: bytes, nom_fichier: str, preview_limit: int = 25) -> dict[str, Any]:
        ...
        return {
            ...
            "normalized_lines_count": len(ai_payload.get("lignes_normalisees", [])),
            "preview_rows_count": len(lignes_preview),
            "parser_rows_count": len(ai_payload.get("classified_rows", [])),
            "governance_rows_count": len(ai_payload.get("classified_rows", [])),
    ...
    def extraire_lignes_normalisees(self, contenu: bytes, nom_fichier: str) -> tuple[list[dict[str, Any]], dict[str, Any]]:
        ...
        return lignes, {
            ...
            "normalized_lines_count": len(lignes),
            "parser_rows_count": len(parse_result["classified_rows"]),
            "governance_rows_count": len(parse_result["classified_rows"]),
            "preview_rows_count": min(len(lignes), 25),
