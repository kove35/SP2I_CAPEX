...
class ServiceAIMapping:
    ...
    def analyser_excel(self, contenu: bytes, nom_fichier: str, preview_limit: int = 25) -> dict[str, Any]:
        ...
        feuilles = self.repository_excel.lire_workbook(contenu)
        ...
        if meilleure_analyse and meilleure_analyse["score_dqe"] > 0:
            lignes_completes = self.extraire_lignes_normalisees_depuis_analyse(
                feuilles[meilleure_analyse["feuille"]],
                meilleure_analyse,
                lineage=lineage,
            )
    ...
    def extraire_lignes_normalisees(self, contenu: bytes, nom_fichier: str) -> tuple[list[dict[str, Any]], dict[str, Any]]:
        ...
        lignes = self.extraire_lignes_normalisees_depuis_analyse(
            feuilles[meilleure_analyse["feuille"]],
            meilleure_analyse,
            lineage=lineage,
        )
